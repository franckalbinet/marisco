...
# %% auto 0
__all__ = ['fname_in', 'fname_out', 'load_to_dataframes', 'ValidateEnumsCB', 'ValidateNetCDFVarsCB', 'ConvertToHumanReadableCB',
           'decode']
    ...
    def __call__(self, tfm: Transformer):
        ...
        with Dataset(self.src_fname, 'r') as nc:
            for group_name, df in tfm.dfs.items():
                ...
                if self.verbose:
                    print ('Processing {group} enums ...')
                
                # Process each variable that has an enum
                for var_name, var in group.variables.items():
                    if hasattr(var.datatype, 'enum_dict'):
                        if self.verbose:
                            print(f"Found enum variable '{var_name}' in group '{group_name}'")

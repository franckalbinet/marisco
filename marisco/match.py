...
# %% auto #0
__all__ = ['uniq_across_dfs', 'lut_from', 'fuzzy_merge', 'fix_lut', 'make_lut_from', 'make_lut', 'parse_ref_ids']
...
def parse_ref_ids(
    ...
    if isinstance(ref_ids, str):
        ...
        except ValueError: raise ValueError(f"Invalid ref_ids: {ref_ids!r}. Expected comma-separated integers, e.g. '16,30'") from None
    if valid is not None:
        unknown = set(ref_ids) - set(valid)
        if unknown: raise ValueError(f"Unknown ref_ids: {sorted(unknown)}. Valid: {sorted(valid)}")
    return ref_ids

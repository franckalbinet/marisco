...
# %% auto #0
__all__ = ['INIS_QA_API', 'INIS_API', 'DEFAULT_ZENODO_RECORDS_URL', 'GlobAttrsFeeder', 'BboxCB', 'DepthRangeCB', 'TimeRangeCB',
           'ZoteroClient', 'ZoteroCB', 'fetch_inis', 'find_curl', 'INISClient', 'InisCB', 'fetch_zenodo', 'ZenodoItem',
           'ZenodoCB', 'KeyValuePairCB']
...
# %% ../nbs/api/metadata.ipynb #990f0c6b
DEFAULT_ZENODO_RECORDS_URL = "https://zenodo.org/api/records"

def fetch_zenodo(
    record_id:str, # Zenodo record identifier
    base_url:str=DEFAULT_ZENODO_RECORDS_URL # API base URL
    ) -> dict: # Raw Zenodo record payload
    "Fetch a Zenodo record from the public API."
    url = f"{base_url}/{record_id}"
    with urlopen(url) as response:
        return json.loads(response.read().decode('utf-8'))

class ZenodoItem:
    "Retrieve Zenodo metadata from the public API."
    def __init__(self,
                 record_id:str, # Zenodo record identifier
                 base_url:str=DEFAULT_ZENODO_RECORDS_URL # API base URL
                ) -> None:
        store_attr()
        try: self.item = fetch_zenodo(record_id, base_url)
        except Exception: self.item = None
    ...
    def _get(self, key:str # Zenodo metadata key to retrieve
            ) -> str:   # Value for that key, or '' if item not loaded
        if not self.item: return ''
        value = self.item.get('metadata', {}).get(key, '') or ''
    ...
    @property
    def summary(self) -> str: return self._get('description')

    @property
    def doi(self) -> str:
        if not self.item: return ''
        doi = self.item.get('doi') or self.item.get('metadata', {}).get('doi', '')
        if isinstance(doi, dict): doi = doi.get('identifier') or ''
        if doi: return doi
        return self.item.get('pids', {}).get('doi', {}).get('identifier', '')
    ...
    def url(self) -> str:
        if not self.item: return ''
        links = self.item.get('links', {})
        return links.get('self_html') or links.get('latest_html') or self.item.get('doi_url', '')

# %% ../nbs/api/metadata.ipynb #18b20788
class ZenodoCB(Callback):
    ...
    def __call__(self, obj):
        item = ZenodoItem(self.record_id, self.base_url)
        if item.exist():

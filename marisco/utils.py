...
# %% auto 0
__all__ = ['get_unique_across_dfs', 'has_valid_varname', 'get_bbox', 'download_files_in_folder', 'download_file', 'match_worms',
           'Match', 'match_maris_lut', 'test_dfs']
...
def get_unique_across_dfs(dfs:dict,  # Dictionary of dataframes
    ...
    unique_values = list(set().union(*(df[col_name].unique() for df in dfs.values())))
    if not as_df:
        return unique_values
    else:
        df = pd.DataFrame(unique_values, columns=['value']).reset_index()
        if include_nchars: df['n_chars'] = df['value'].str.len()
        return df
